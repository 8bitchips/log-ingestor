...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://sks9234696566:8JaW6FcShmET@ep-flat-resonance-40823516.us-east-2.aws.neon.tech/logs?sslmode=require'
...
def get_logs():
    try:
        # Retrieve the page and per_page parameters from the request
        page = int(request.args.get('page', 1))
        per_page = int(request.args.get('per_page', 10))  # You can adjust the default per_page value

        # Calculate the offset based on page and per_page
        offset = (page - 1) * per_page

        # Retrieve logs based on pagination
        with app.app_context():
            logs = Log.query.order_by(Log.timestamp.desc()).offset(offset).limit(per_page).all()
...
def query_logs():
    try:
        ...
        if filters:
            ...
            logs = base_query.filter(dynamic_filter).order_by(Log.timestamp.desc()).offset(offset).limit(per_page).all()
        else:
            logs = base_query.order_by(Log.timestamp.desc()).offset(offset).limit(per_page).all()
